...
class FBMatchForm : AppCompatActivity() {
    ...
    fun validateInputs() : Boolean {
        ...
        if (pname1_field.text.isNullOrEmpty()) {
            Toast.makeText(this@FBMatchForm,"Please fill Player 1 Name", Toast.LENGTH_LONG).show()
            valid = false
        } else if (pname2_field.text.isNullOrEmpty()) {
            Toast.makeText(this@FBMatchForm,"Please fill Player 2 Name", Toast.LENGTH_LONG).show()
            valid = false
        } else if (round_field.text.isNullOrEmpty() || round_field.text.toString().toInt() < 0) {
            Toast.makeText(this@FBMatchForm,"Rounds must be a valid number", Toast.LENGTH_LONG).show()
            valid = false
        }
        return valid
    }
